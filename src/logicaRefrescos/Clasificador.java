...
public class Clasificador {
	...
	/*
	 * Atributos de la clase int saldo - saldo que ha introducido el usuario
	 * HashMap<String, Dispensador> misDispensadores - contiene todos los
	 * dispensadores (uno por producto de la maquina) HashMap<Integer, Deposito>
	 * - depositos de las monedas
	 */
	private int saldo; // En centimos
	private String mensaje; // Para guardar el "estado" y poder mostrarlo al
							// usuario por pantalla
	private HashMap<String, Dispensador> misDispensadores;
	private HashMap<Integer, Deposito> misDepositos; // uno por cada moneda:
														// 1,2,5,10,20,50,1,2
														// (ampliar)
	 ...
	 */
	public Clasificador(HashMap<Integer, Deposito> hmDepositos, HashMap<String, Dispensador> hmDispensadores) {
		saldo = 0;
	 ...
	 * Añade una moneda al deposito correspondiente, aumenta el saldo y llama
	 * al visor
	 * 
	...
	/*
	 * Devuelve las monedas necesarias (restar de cada deposito) El saldo final
	 * debe ser 0 Se puede llamar desde un objeto retorno o desde esta propia
	 * clase al seleccionar un producto si hay que dar cambio Hay que comprobar
	 * que hay saldo. Si no hay saldo mostrar error
	 */
	public boolean retornarMonedas() {
		...
		if (saldo == 0) {
			...
			/*
			 * Calculo de las monedas que hay que devolver Empezamos por la mas
			 * alta y vamos devolviendo si hay disponibles
			 */
			...
			if (saldo > 0) {
				...
				auxMensaje += "\nLlame al telefono gratuito 900 100 800 para poner una reclamacion por los " + saldo
						+ " centimos que no le han sido devueltos";
	...
	public boolean seleccionarProducto(String clave) {
		/*
		 * Si no hay disponiblidad => Mostramos error "Producto agotado" Si
		 * saldo = 0 => Mostramos precio Si saldo > 0 y saldo < precio =>
		 * Mostramos error "Saldo insuficiente: se necesitan X euros" Si no hay
		 * cambio => Mostramos error "No hay monedas para proporcionar cambio"
		 * Si saldo > precio y hay cambio => Dispensamos producto, retornamos
		 * monedas y mostramos mensaje despedida
		 */
			...
			if (saldo == 0) {
				...
				if (saldo < precio) {
					...
					if (!hayCambio(saldo - precio)) {
						...
						saldo = saldo - precio;
						// Hacemos la devolución de monedas desde el
						// controlador para poder controlar la salida de la lata
						// if (saldo > 0) {
